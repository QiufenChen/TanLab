...
with landscape:
    ...
    if not filtered_df.empty:
        ...
        fig.add_shape(type="line",
                x0=0, y0=0, x1=0, y1=max(filtered_df['Pvalue']),
...
with network:
    ...
    if not filtered_df.empty:
        ...
        # 添加节点和边 'Drugs', 'Target', 'Hscore', 'logFC', 'Pvalue'
        ...
        for edge in G.edges(data=True):
            ...
            # norm_weights = (edge[2]['weight'] - min_weight) / (max_weight - min_weight)
            # rgba_color = cmap(norm_weights)
            # hex_color = '#{:02X}{:02X}{:02X}'.format(int(rgba_color[0]*255), int(rgba_color[1]*255), int(rgba_color[2]*255))
            # # hex_color = 'rgba({}, {}, {}, {})'.format(int(rgba_color[0]*255), int(rgba_color[1]*255), int(rgba_color[2]*255), rgba_color[3])
            # print(hex_color)
            # edge_trace['line']['color'].append(hex_color)
            # edge_trace.line.color += tuple([color])

            # edge_trace['line']['color'] += [edge[2]['weight']]
